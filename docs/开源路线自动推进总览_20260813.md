...
| 演示检查 | 17/17 | 主要允许、暂停、拒绝、篡改、重放、KMS/共享票据和原生隔离路径可复现 | 不是未知攻击或压力测试 |
| OpenBao外部密钥/共享核销 | 7/7 | 密钥外置、轮换、双网关共享状态和32路并发核销 | 本机dev server不是多节点生产集群 |
...
| B 当前原型 | 完整一键回归 | 已完成 | 31/31、55/55、46/46、网络5/5、身份5/5、OpenBao7/7、QEMU9/9 | 每次变更自动回归并保留时间戳 |
...
| F 状态与密钥 | 外部密钥与共享票据状态 | 测试级完成 | OpenBao Transit+KV 7/7 | 生产补多节点Raft/云KMS故障切换 |
