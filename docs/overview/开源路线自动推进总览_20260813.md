...
> 基线更新：2026-08-14；OpenClaw 模型回环补充：2026-08-27
...
| Python安全测试 | 174/174（以最新实际报告为准） | 身份、审批、网关、适配器、OpenClaw MCP、容器后端票据校验、内核、公开基准适配与阶段4预检回归通过 | 不等于外部生产系统E2E |
...
| 演示检查 | 23/23 | 主要允许、暂停、拒绝、篡改、重放、签名审批、KMS/共享账本、Raft故障切换和原生隔离路径可复现 | 不是未知攻击或压力测试 |
...
| OpenClaw模型回环 | 固定5例 synthetic fixture 5/5；CLI 14/14；Control UI 16/16 | 当前隔离配置中模型只调用唯一允许的只读 AgentGuard MCP 工具 | 回环静态开发身份和合成数据；不是公开基准或生产验收 |
...
- 单项复测 Keycloak 7/7；随后完整一键回归通过，加入公开基准、阶段4对抗门禁和 OpenClaw MCP 适配器后 Python 测试增至174。
...
| B 当前原型 | 完整一键回归 | 已完成 | 31/31、55/55、174/174、网络5/5、身份7/7、OpenBao10/10、Raft HA 8/8、QEMU11/11、OpenClaw接入9/9、模型回环5/5、综合检查23/23 | 每次变更自动回归并保留时间戳 |
