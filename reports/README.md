...
| [`e2e/openclaw/`](e2e/openclaw/) | 项目内 OpenClaw 安装、Control UI、MCP 工具发现、低风险协议调用和真实模型工具回合 | `openclaw_installation.json`、`openclaw_agentguard_visual_demo.json`、`openclaw_agentguard_control_ui_turn.json`、`openclaw_mcp_integration.json`、`openclaw_agentguard_model_turn.json` |
...
- `openclaw_agentguard_visual_demo.*` 只证明 Gateway、Control UI 页面、MCP 注册与 schema；`openclaw_agentguard_control_ui_turn.*` 证明已认证网页会话中可见真实工具调用；`openclaw_agentguard_model_turn.*` 提供 CLI agent 转录的独立模型调用证据。两种模型回合仍使用回环静态测试身份和隔离合成数据，不是生产验收。
